...
namespace OpenCLBenchmark
{
	public class OpenClKernelCompiler
	{
		...
		public void FillKernelsCombobox(ComboBox comboBox, string wildcard = "")
		{
			...
			// Only keep files that match the wildcard
			if (!string.IsNullOrEmpty(wildcard))
			{
				kernelFiles = kernelFiles.Where(kvp => kvp.Value.Contains(wildcard, StringComparison.OrdinalIgnoreCase)).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
			}
